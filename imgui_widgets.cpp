...
bool ImGui::Tab(const char* label, const char* icon, const char* ksub, bool selected)
{
    ...
    if (selected)
    {
        ...
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(accent_colour[0], accent_colour[1], accent_colour[2], 1.f));
    ...
    {
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.69f, 0.69f, 0.69f, 1.00f));
        ImGui::PushFont(info);
        ImGui::RenderText(ImVec2(bb.Min.x + 30, bb.Min.y + (size.y / 2) - (label_size.y / 2)), label);
        ImGui::PopStyleColor();
        ImGui::PopFont();
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (ImGui::IsItemHovered())
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.12f, 0.20f, 0.32f, 1.f), ImVec4(accent_colour[0], accent_colour[1], accent_colour[2], 1.f), t));
    else
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.12f, 0.20f, 0.32f, 1.f), ImVec4(accent_colour[0], accent_colour[1], accent_colour[2], 1.f), t));

    draw_list->AddRectFilled(ImVec2(p.x + 18, p.y + 3), ImVec2(13 + p.x + width, p.y + height - 3), col_bg, height * 0.5f);
    draw_list->AddCircleFilled(ImVec2(15 + p.x + radius + t * (width - radius * 2.0f), p.y + radius - -0.5f), radius - 2.5f, ImColor(255, 255, 255));
    ...
    if (label_size.x > 0.0f)
    {
        if (*v)
        {
            ...
            ImGui::RenderText(ImVec2((p.x + width) + 20, (p.y + height) - 18), label);
...
P p[64];
void ImGui::Particles(ImDrawList* d, ImVec2 o, ImVec2 b, ImVec2 sz, ImVec4, float t)
{
    int j{};
    ...
    for (P& a : p) {
        ...
        ImColor col = ImVec4(accent_colour[0], accent_colour[1], accent_colour[2], 1.f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(160, 20));
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Max.x, frame_bb.Max.y), ImColor(14, 14, 14), style.ComboRounding, popup_open ? 3 : 15);

    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        RenderArrow(ImVec2(frame_bb.Max.x - arrow_size + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + 1.5f), ImGuiDir_Down);
    }

    

    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(ImVec2(frame_bb.Min.x + 5, frame_bb.Min.y + 2), value_bb.Max, preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(160, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x + 0, frame_bb.Min.y + 10), ImVec2(frame_bb.Max.x, frame_bb.Min.y + 10), ImColor(50, 50, 58), 3);
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x + 0, frame_bb.Min.y + 10), ImVec2(grab_bb.Min.x, frame_bb.Min.y + 10), ImColor(0, 150, 255), 3);
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x - 15, frame_bb.Min.y + 2), ImVec2(frame_bb.Min.x - 15, frame_bb.Max.y - 2), ImColor(35, 35, 35));
    window->DrawList->AddLine(ImVec2(frame_bb.Max.x + 15, frame_bb.Min.y + 2), ImVec2(frame_bb.Max.x + 15, frame_bb.Max.y - 2), ImColor(35, 35, 35));
    window->DrawList->AddLine(ImVec2(frame_bb.Max.x + 150, frame_bb.Min.y + 2), ImVec2(frame_bb.Max.x + 150, frame_bb.Max.y - 2), ImColor(35, 35, 35));

    // Render grab
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x, grab_bb.Min.y + 4), ImVec2(grab_bb.Max.x - 2, grab_bb.Max.y - 3), GetColorU32(ImGuiCol_SliderGrab), 12);
    ...
    ImGui::PushFont(info);
    RenderTextClipped(ImVec2(frame_bb.Max.x + 150, frame_bb.Min.y), ImVec2(frame_bb.Max.x + 180, frame_bb.Max.y - 2), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    ImGui::PopFont();
    ImGui::PopStyleColor();

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + 25, frame_bb.Min.y + 1), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (!is_multiline)
    {
        ...
        draw->AddRect(frame_bb.Min, frame_bb.Max, ImColor(35, 35, 35), 9);
    }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_rgb_without_alpha), 3, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_BotLeft);
    }
    else
    {
        ...
        if (col_source.w < 1.0f)
            ...
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 3, ImDrawCornerFlags_All);
    }
    RenderNavHighlight(bb, id);
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            ...
            window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), 3); // Color button are often in need of some sort of border
    }
...
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(326, 35), flags, callback, user_data);
}
